<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<!DOCTYPE html>
<html lang="es">
	<head>
		<title>Depósito</title>
		<link href="resources/css/bootstrap.min.css" rel="stylesheet" />
	</head>
	
	<body>
		<nav class="navbar navbar-inverse navbar-fixed-top">
		  <div class="container">
			<div class="navbar-header">
				<a class="navbar-brand" href="#">Solicitudes de Artículos Pendientes</a>
			</div>
		  </div>
		</nav>
		
		<div class="col-sm-8 col-md-8 col-lg-8 col-md-offset-2" style="margin-top:100px">
			<table id="listaArticulos" class="table table-striped">
				<thead>
					<tr>
						<th>Fecha de Emisión</th>
						<th>Artículos solicitados</th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${solicitudesArticulosPendientes}" var="solicitud">
						<tr id="articulo_${solicitud.id}">
							<td style="width:200px;">${solicitud.fechaEmision}</td>
							<td>
								<c:forEach items="${solicitud.itemSolicitudArticulo}" var="item">
									<div class="well" style="height:165px">
										<div class="col-sm-12 col-md-12 col-lg-12">
											<div class="col-sm-12 col-md-12 col-lg-12">
												<h4>Artículo: ${item.articulo.codigo} - ${item.articulo.nombre}</h4>
											</div>
											<div class="col-sm-10 col-md-10 col-lg-10 col-md-offset-2">
												Stock Actual: ${item.articulo.stock}
											</div>
											<div class="col-sm-10 col-md-10 col-lg-10 col-md-offset-2">
												Stock Solicitado: ${item.cantidad}
											</div>
										</div>
										<div class="col-sm-12 col-md-12 col-lg-12">
											<div class="col-sm-3 col-md-3 col-lg-3">
												Stock a solicitar: 
												<input name="" value="${item.cantidad*2}" class="form-control form-group">								
											</div>
										</div>
									</div>
								</c:forEach>
							</td>
							<td style="width:170px;">
								<button class="btn btn-primary">
									<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true" style="font-size:20px; cursor:pointer"></span>
									Enviar Solicitud
								</button>
							</td>
						</tr>					
					</c:forEach>
				</tbody>
			</table>
		</div>
		
		
		<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>		
		<script src="resources/js/vista/bootstrap.min.js"></script>
	</body>
</html>